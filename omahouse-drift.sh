...
# {"Name":"omahouse-drift","Description":"Is this machine running the profiles it is supposed to be?","Tags":["omahouse","read","cue","fleet"],"Fields":[{"Name":"source","Prompt":"Path to the intended profiles.json","Type":"string","Order":1,"Required":false,"Arg":"--source","Default":""}]}
# OMAKURE_SCHEMA_END
...
# A read whose whole answer fits in an exit code, which is why it is one of the
# two questions this design can ask remotely at all. Nothing useful is printed
# to a dispatched run and nothing needs to be: the Health Plane forbids stdout,
# and the number is the finding.
#
#   0  this machine is running its intended profiles
#   2  one of the two files is not there
#   3  they differ
#
# Compared over bytes. omahouse writes profiles.json with sorted keys, so the
# intended file should be what omahouse wrote rather than something
# hand-formatted -- two spellings of one meaning read as drift here, and that is
# the honest failure for a check this cheap.
live=${OMAHOUSE_CONFIG_DIR:-/etc/omahouse}/profiles.json

[[ -e $live ]] || die "no $live on this machine" "$EX_MISSING"
[[ -e $source ]] || die "no intended profiles at $source" "$EX_MISSING"

# Readable by anybody: omahouse writes profiles.json 0644 on purpose, so this
# check needs no privilege and is safe to leave dispatchable.
if cmp -s -- "$live" "$source"; then
    printf 'in step with %s\n' "$source"
    exit "$EX_OK"
fi

printf 'this machine has drifted from %s\n' "$source" >&2
exit "$EX_NO"
