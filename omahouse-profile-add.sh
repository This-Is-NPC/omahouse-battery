...
# {"Name":"omahouse-profile-add","Description":"Put an account under rules, and make it if it is not there","Tags":["omahouse","write","cue"],"Fields":[{"Name":"user","Prompt":"Which account","Type":"string","Order":1,"Required":false,"Arg":"--user","Default":""},{"Name":"name","Prompt":"The name to show, like Julia","Type":"string","Order":2,"Required":false,"Arg":"--name","Default":""},{"Name":"create-user","Prompt":"Make the Linux account too","Type":"bool","Order":3,"Required":false,"Arg":"--create-user","Default":"false"}]}
# OMAKURE_SCHEMA_END
...
# Provisioning, which is the one thing `omahouse-profile-apply.sh` cannot do.
# That script writes profiles.json, and omahouse will happily hold a profile for
# an account that does not exist -- a rule may be written before its account and
# outlive it. But the person cannot log in, so a machine given rules and no
# account is a machine that looks managed and is not.
#
# `create-user` defaults to **false** on purpose. Of the two ways to be wrong,
# not making an account leaves somebody unable to log in, which is loud and
# harmless; making one nobody asked for adds a person to a computer. A
# dispatched run uses the default, so the safe direction is the one that does
# less.
user=$(settled OMAHOUSE_BATTERY_USER --user "$@")
[[ -n $user ]] || die "no account named. Pass --user, or set OMAHOUSE_BATTERY_USER"
name=$(settled OMAHOUSE_BATTERY_NAME --name "$@")
create=$(settled OMAHOUSE_BATTERY_CREATE_USER --create-user "$@")
create=${create:-false}

set -- profile add "$user"
[[ -n $name ]] && set -- "$@" --name "$name"
[[ $create == true ]] && set -- "$@" --create-user
