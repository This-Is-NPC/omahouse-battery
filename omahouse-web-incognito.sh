...
# {"Name":"omahouse-web-incognito","Description":"Whether a private window may open at all","Tags":["omahouse","write","cue"],"Fields":[{"Name":"user","Prompt":"Which account, or empty to take the machine's own","Type":"string","Order":1,"Required":false,"Arg":"--user","Default":""},{"Name":"state","Prompt":"allow or deny","Type":"string","Order":2,"Required":false,"Arg":"--state","Default":"deny","Choices":["allow","deny"]}]}
# OMAKURE_SCHEMA_END
...
# A private window is a window every site rule is blind to, so this is the
# switch that decides whether the rest of the web half means anything.
#
# `deny` by default, and it asks the browser for something rather than nothing:
# allowing incognito writes no policy at all, because a browser left alone
# already allows it.
...
case $state in
    allow) omahouse_root web incognito "$user" --allow ;;
    deny)  omahouse_root web incognito "$user" --deny ;;
    *)     die "state is allow or deny, not '$state'" ;;
esac
