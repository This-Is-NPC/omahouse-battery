...
# {"Name":"omahouse-profile-default","Description":"Whether an unnamed program may open, or may not","Tags":["omahouse","write","cue"],"Fields":[{"Name":"user","Prompt":"Which account, or empty to take the machine's own","Type":"string","Order":1,"Required":false,"Arg":"--user","Default":""},{"Name":"verdict","Prompt":"allow or deny","Type":"string","Order":2,"Required":false,"Arg":"--verdict","Default":"deny","Choices":["allow","deny"]}]}
# OMAKURE_SCHEMA_END
...
# What happens to a program no rule names. `deny` makes the rules an allowlist,
# `allow` makes them a denylist, and it is the same engine either way -- half of
# what a profile means lives in this one word.
#
# The default is `deny`, the restrictive side, for the reason `omahouse-web-rule`
# defaults to `block`: a dispatched run uses its default, so a stale Battery or a
# Cue that reached the wrong machine lands on the side that takes something away
# rather than opening a machine up.
user=$(the_account "$(settled OMAHOUSE_BATTERY_USER --user "$@")")
verdict=$(settled OMAHOUSE_BATTERY_VERDICT --verdict "$@")
verdict=${verdict:-deny}

case $verdict in
    allow) omahouse_root profile default "$user" --allow ;;
    deny)  omahouse_root profile default "$user" --deny ;;
    *)     die "verdict is allow or deny, not '$verdict'" ;;
esac
