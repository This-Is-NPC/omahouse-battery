...
# {"Name":"omahouse-health","Description":"Is the daemon up and did it count today? The exit code is the answer","Tags":["omahouse","read","cue"],"Fields":[{"Name":"stale-after","Prompt":"Seconds a ledger may go unwritten before it counts as stale","Type":"number","Order":1,"Required":false,"Arg":"--stale-after","Default":"300"}]}
# OMAKURE_SCHEMA_END
...
# The question a household with more than one machine cannot answer today, in
# the one shape that can travel: a number.
#
#   0  the daemon is up, and either it is counting or there is nobody to count
#   2  omahouse is not installed here
#   3  the daemon is down, or it has a profile to watch and has counted nothing
#
# "Up" is not enough on its own. A daemon that is running and has stopped
# writing is the failure this is for, and it looks identical to a healthy one
# from `systemctl` alone.
#
# `stale-after` keeps a default because five minutes is a judgement about a two
# second loop, not a fact about this machine.
stale=$(settled OMAHOUSE_BATTERY_STALE_AFTER --stale-after "$@")
stale=${stale:-300}
[[ $stale =~ ^[0-9]+$ ]] || die "stale-after is a whole number of seconds, not '$stale'"

bin >/dev/null

if ! systemctl is-active --quiet omahouse.service; then
    printf 'omahouse.service is not active\n' >&2
    exit "$EX_NO"
fi

config=${OMAHOUSE_CONFIG_DIR:-/etc/omahouse}
state=${OMAHOUSE_STATE_DIR:-/var/lib/omahouse}

# No profiles is not a fault. It is the state of every machine before the first
# rule is written, and a check that called it unhealthy would cry on every
# machine that has omahouse installed and nobody under rules yet.
if [[ ! -s $config/profiles.json ]]; then
    printf 'up, and nobody is under rules on this machine\n'
    exit "$EX_OK"
fi

today=$(date +%Y-%m-%d)
newest=0
for ledger in "$state"/*/"$today.json"; do
    [[ -e $ledger ]] || continue
    written=$(stat -c %Y "$ledger")
    (( written > newest )) && newest=$written
done

# A day nobody has spent has no file, and that is legible rather than broken:
# nobody has logged in yet. The daemon being active is the whole of what can be
# asserted then.
if (( newest == 0 )); then
    printf 'up, and no session has been counted yet today\n'
    exit "$EX_OK"
fi

age=$(( $(date +%s) - newest ))
if (( age > stale )); then
    printf 'up, but the newest ledger has not been written for %ss\n' "$age" >&2
    exit "$EX_NO"
fi

printf 'up and counting; newest ledger written %ss ago\n' "$age"
exit "$EX_OK"
