...
# {"Name":"omahouse-profile-remove","Description":"Take an account out from under the rules","Tags":["omahouse","write","cue"],"Fields":[{"Name":"user","Prompt":"Which account","Type":"string","Order":1,"Required":false,"Arg":"--user","Default":""},{"Name":"keep-account","Prompt":"Leave the Linux account alone","Type":"bool","Order":2,"Required":false,"Arg":"--keep-account","Default":"true"}]}
# OMAKURE_SCHEMA_END
...
# The other end of `omahouse-profile-add.sh`, and the more dangerous one, so it
# is the narrower.
#
# `keep-account` defaults to **true** and has to be argued out of. Taking the
# rules off somebody is undoing a decision; taking their account away is undoing
# their afternoon, their files and their session, and a Cue that could do the
# second by default would be a remote channel that can empty a computer.
user=$(settled OMAHOUSE_BATTERY_USER --user "$@")
[[ -n $user ]] || die "no account named: remove refuses to guess. \
Pass --user, or set OMAHOUSE_BATTERY_USER"
keep=$(settled OMAHOUSE_BATTERY_KEEP_ACCOUNT --keep-account "$@")
keep=${keep:-true}

if [[ $keep == true ]]; then
    omahouse_root profile remove "$user" --keep-account
else
    omahouse_root profile remove "$user"
fi
