...
# {"Name":"omahouse-status","Description":"The live picture for one account, as JSON (local only)","Tags":["omahouse","read","local-only"],"Fields":[{"Name":"user","Prompt":"Which account, or empty to take the machine's own","Type":"string","Order":1,"Required":false,"Arg":"--user","Default":""}]}
# OMAKURE_SCHEMA_END
...
# Local only, and that is a property of Omakure rather than a shortcoming here.
# What this prints -- the account's name, the programs it has open, what it did
# today -- is precisely the class the Health Plane refuses to carry: script
# stdout, usernames, process lists, user activity, all forbidden at any depth.
# A Cue could start this and would bring back nothing but the exit code.
#
# Run it from the machine's own CLI, or through its authenticated HTTP API.
# Reading needs no privilege in omahouse, so this deliberately does not reach
# for root: the fiscalised person runs the same command to see their own day,
# and a script that needed sudo to answer would be one they could not.
user=$(the_account "$(settled OMAHOUSE_BATTERY_USER --user "$@")")
b=$(bin)

set +e
out=$("$b" status "$user" --json)
code=$?
set -e
printf '%s\n' "$out"
# omahouse's own codes, passed through rather than flattened: 2 is "no such
# account or profile", which a caller has to be able to tell from a real fault.
exit "$code"
