...
# {"Name":"omahouse-web-rule","Description":"Block or allow one site for a profile","Tags":["omahouse","write","cue"],"Fields":[{"Name":"user","Prompt":"Which account, or empty to take the machine's own","Type":"string","Order":1,"Required":false,"Arg":"--user","Default":""},{"Name":"verdict","Prompt":"block or allow","Type":"string","Order":2,"Required":false,"Arg":"--verdict","Default":"block","Choices":["block","allow"]},{"Name":"domain","Prompt":"The bare domain, which covers its subdomains","Type":"string","Order":3,"Required":false,"Arg":"--domain","Default":""}]}
# OMAKURE_SCHEMA_END
...
# The rule goes into the profile and nothing here touches the browser. The
# daemon composes every profile's web rules into one Chromium managed policy on
# its next cycle, and it is the only writer of that file -- a script that edited
# the policy directly would be a second answer to the same question, and the two
# would eventually disagree about a machine nobody was watching.
#
# `verdict` keeps a default, unlike a program's, because here one side is safe:
# a `--verdict` lost in transit lands on `block`, which takes something away
# rather than opening something. A program verdict has no safe side.
user=$(the_account "$(settled OMAHOUSE_BATTERY_USER --user "$@")")
verdict=$(settled OMAHOUSE_BATTERY_VERDICT --verdict "$@")
verdict=${verdict:-block}
domain=$(settled OMAHOUSE_BATTERY_DOMAIN --domain "$@")

[[ -n $domain ]] || die "no site named. Pass --domain, or set OMAHOUSE_BATTERY_DOMAIN"
[[ $domain != *://* ]] || die "a bare domain, not a URL: '$domain'"

case $verdict in
    block) omahouse_root web block "$user" "$domain" ;;
    allow) omahouse_root web allow "$user" "$domain" ;;
    *)     die "verdict is block or allow, not '$verdict'" ;;
esac
