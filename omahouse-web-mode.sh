...
# {"Name":"omahouse-web-mode","Description":"Whether the web rules are a list of what opens or of what does not","Tags":["omahouse","write","cue"],"Fields":[{"Name":"user","Prompt":"Which account, or empty to take the machine's own","Type":"string","Order":1,"Required":false,"Arg":"--user","Default":""},{"Name":"mode","Prompt":"only-listed or all-but-listed","Type":"string","Order":2,"Required":false,"Arg":"--mode","Default":"only-listed","Choices":["only-listed","all-but-listed"]}]}
# OMAKURE_SCHEMA_END
...
# The same choice `omahouse-profile-default.sh` makes about programs, made about
# sites: `only-listed` is an allowlist and `all-but-listed` is a denylist.
#
# `only-listed` is the default because it is the restrictive one. And it reaches
# further than the account it is written for: the browser policy is one file for
# the whole machine, so an allowlist closes the web for every account on it,
# yours included. omahouse says so when you write it, and a dispatched run has
# nobody to say it to.
user=$(the_account "$(settled OMAHOUSE_BATTERY_USER --user "$@")")
mode=$(settled OMAHOUSE_BATTERY_MODE --mode "$@")
mode=${mode:-only-listed}

case $mode in
    only-listed)     omahouse_root web "$user" --only-listed ;;
    all-but-listed)  omahouse_root web "$user" --all-but-listed ;;
    *) die "mode is only-listed or all-but-listed, not '$mode'" ;;
esac
