...
# {"Name":"omahouse-enforce","Description":"Put the teeth in, or take them out","Tags":["omahouse","write","cue"],"Fields":[{"Name":"user","Prompt":"Which account, or empty to take the machine's own","Type":"string","Order":1,"Required":false,"Arg":"--user","Default":""},{"Name":"state","Prompt":"on or off","Type":"string","Order":2,"Required":false,"Arg":"--state","Default":"off","Choices":["on","off"]}]}
# OMAKURE_SCHEMA_END
...
# The narrowest script here, and the most dangerous, which are the same fact.
# `on` is what makes omahouse close programs and end sessions; `off` leaves it
# counting and warning and carrying nothing out.
#
# The default is `off` and not `on`, and it stays a default rather than becoming
# a required value: a dispatched run uses it, so a stale Battery version, a
# mistake in the trust registry or a Cue that reached the wrong machine all land
# on the side that takes nothing away.
user=$(the_account "$(settled OMAHOUSE_BATTERY_USER --user "$@")")
state=$(settled OMAHOUSE_BATTERY_STATE --state "$@")
state=${state:-off}

case $state in
    on)  omahouse_root profile enforce "$user" --on ;;
    off) omahouse_root profile enforce "$user" --off ;;
    *)   die "state is on or off, not '$state'" ;;
esac
