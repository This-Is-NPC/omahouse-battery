...
# {"Name":"omahouse-profile-show","Description":"The rules of one profile, as JSON (local only)","Tags":["omahouse","read","local-only"],"Fields":[{"Name":"user","Prompt":"Which account, or empty to take the machine's own","Type":"string","Order":1,"Required":false,"Arg":"--user","Default":""}]}
# OMAKURE_SCHEMA_END
...
# What the rules actually say, as opposed to what somebody remembers writing.
#
# Local only, like `status` and `report` and for the same reason: the account's
# name is in the answer, and the Health Plane forbids usernames at any depth. A
# Cue could start this and would bring back nothing but the exit code.
#
# Reading needs no privilege at all -- profiles.json is 0644 on purpose -- so
# this deliberately does not reach for root.
user=$(the_account "$(settled OMAHOUSE_BATTERY_USER --user "$@")")
b=$(bin)

set +e
out=$("$b" profile show "$user" --json)
code=$?
set -e
printf '%s\n' "$out"
# omahouse's own codes, passed through: 2 is "no such account or profile", which
# a caller has to be able to tell from a real fault.
exit "$code"
