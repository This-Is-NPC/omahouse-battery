...
# {"Name":"omahouse-deny","Description":"Take a program back off the list","Tags":["omahouse","write","cue"],"Fields":[{"Name":"user","Prompt":"Which account, or empty to take the machine's own","Type":"string","Order":1,"Required":false,"Arg":"--user","Default":""},{"Name":"app","Prompt":"The scope id of the program","Type":"string","Order":2,"Required":false,"Arg":"--app","Default":""}]}
# OMAKURE_SCHEMA_END
...
# Its own script and not a flag on `allow`, deliberately. A Cue names a script
# and cannot configure it, so a single script taking `--allow` or `--deny` would
# be one whose default decides which of two opposite things a fleet does. Two
# files, two names, no default to get wrong.
user=$(the_account "$(settled OMAHOUSE_BATTERY_USER --user "$@")")
app=$(settled OMAHOUSE_BATTERY_APP --app "$@")

[[ -n $app ]] || die "no program named: deny refuses to guess. \
Pass --app, or set OMAHOUSE_BATTERY_APP"

omahouse_root deny "$user" "$app"
